...
- `miniprogram/pages/review/`：“复习”Tab，本地资料库合并自动沉淀的生词与用户标记的难句；生词可直接听发音，难句按课程时间轴在页内播放单句原声，来源课程只作为上下文展示，不把整卡做成隐式跳转。
...
8. 首页场景搜索只过滤已经加载的章节树，不产生额外请求；查词结果与难句正文写入本地 `waimao_review_library_v1`，不上传录音或复习正文。复习页播放难句时按需读取课程详情，用 cue `start/end` 限制为单句原声，离开页面立即停止。
