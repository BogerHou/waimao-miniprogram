...
- `miniprogram/pages/coach/`：实验版默认首页，承载今日、场景、复习和我的四个主视图。
...
1. `pages/coach` 复用课程列表、后端进度、entitlement 和 app config，默认只推荐当前可访问场景。
...
4. 用户自评、复习到期时间和录音路径保存在 `waimao_coach_progress_v1` 本地存储；录音通过 `wx.saveFile` 留在本机，新录音会替换并清理同句旧文件。
5. 已登录用户仍通过现有进度接口同步当前 cue 和场景完成状态；表达自评与录音当前不上传，也不宣称可以跨设备恢复。
6. 训练完成后，今日页显示 100% 状态，复习页只列出“需要巩固”或已到复习时间的表达。
