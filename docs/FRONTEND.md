...
- `pages/index`：章节首页。只负责展示 7 章、小节锁定和解锁提示；不做学习统计仪表盘。
- `pages/course`：练习详情。不要随意改动播放控制栏、字幕列表、AI 弹层、查词交互。
- `pages/knowledge`：外贸知识点新页面，用于替代课程页悬浮知识点按钮的旧 PDF 图片入口。
- `pages/unlock`：会员邀请码解锁页；入口要求用户填写头像/昵称登录，页面简洁展示添加微信二维码、全部章节 1 年访问权、邀请码输入和解锁按钮。

## 设计约束

- 配色沿用 EnglishPod：首页使用紫蓝主色 `#6366f1/#818cf8`，课程详情使用蓝色 `#3B82F6`，背景和文字使用 EnglishPod 原有浅蓝灰体系。
- 首页保持 EnglishPod 的“个人卡片 + 提示位 + 列表”结构，章节和小节列表只展示进入训练所需的信息；首屏不展示连续学习天数。
