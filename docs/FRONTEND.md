...
- `pages/course`：练习详情。不要随意改动播放控制栏、字幕列表、AI 弹层、查词交互；不展示手动“完成课程”按钮，播放到小节末尾后自动同步完成状态；字幕按句末标点拆成短段，同一说话人保持同一段落颜色。
...
- 课程详情页和知识点页共用对话拆句规则；课程页只改变单条字幕内部的阅读展示，不拆分或改变播放 cue。
