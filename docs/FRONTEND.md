...
- `pages/unlock`：会员邀请码解锁页；入口要求用户填写头像/昵称登录，页面优先展示会员权益，再展示购买微信二维码、邀请码输入和解锁按钮；顶部返回和标题栏必须在内容滚动时保持可见。
...
- 课程详情页和知识点页共用对话拆句规则；课程页优先使用后端按词级 SRT 生成的短 cue 时间，缺失时才在前端按句子长度估算兜底，保证点击短句时播放对应短句范围。
