...
- 登录用户能写入小节进度、学习时长和头像。
- 学习时长上报必须同时携带本次精练次数；学习主页能从 `/api/waimao-mini/users/me/study-records` 恢复最近 4 周按日数据，接口失败时不得清空本地用户摘要或复习资料。
...
- 学习记录接口已于 2026-07-12 部署并完成登录态联调；体验版仍需在真机核对最近 4 周日历、精练次数与实际练习一致。
