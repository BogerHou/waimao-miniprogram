...
- 场景教练能从上次阶段和 cue 继续；训练清单旧缓存能安全迁移，完成场景后应自动移出清单，用户可以继续选择场景或回到学习台；待巩固表达出现在复习页。
- 场景完整听力和连续跟读必须停在当前小节范围；退出训练页后不得留下播放任务或音频上下文运行错误。
- 麦克风未授权或录音失败时必须保留未录音训练路径，并给出可恢复的权限提示。
