...
2. 微信隐私协议合规：头像/昵称 + 录音权限的隐私弹窗（`wx.requirePrivacyAuthorize` / 隐私接口），审核材料与当前发布级开关状态保持一致。
...
1. 生词本（查词沉淀 + 列表 + 发音 + 回源句自动播放）——与难句星标合并为统一“复习”入口（见下方新增点）。
2. 场景关键词搜索（首页纯前端过滤）。
...
- [x] 阶段 A：release-compliance 全部完成（主题收尾 + 隐私权限流程 + 头像压缩 + 微信后台隐私与发布合规配置）；计划于 2026-07-13 归档。
- [x] 阶段 B：observability + 网络健壮性代码完成（2026-07-12）；当前关闭态真机验收已通过，音源回退 smoke 留待交互能力重新开启前执行，部署后日志采集仍需持续确认。
- [ ] 阶段 C：转化漏斗（2026-07-12 用户决定暂缓，不阻塞阶段 D）。
- [x] 阶段 D：留存闭环及“课程 / 复习 / 我的”三 Tab 信息架构完成（2026-07-12）；当前关闭态的 Tab、搜索、复习、回源定位、学习记录和重新登录收藏保全已于 2026-07-13 完成真机 smoke，音频相关路径留待开关重启前验证。
- [x] 阶段 E：课程词典、死代码与海报清理、产物一致性门禁完成（2026-07-12）；当前关闭态查词正文链路已完成真机检查，英美发音留待开关重启前验证。
