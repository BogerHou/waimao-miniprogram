...
### 🔧 Follow-up: 解锁页权益展示和后台邀请码管理

- 解锁页去掉意义不明的数字指标卡，改为先展示完整课程、1 年访问权和完整跟读体验三项会员权益，再展示购买二维码和邀请码输入。
- 确认小程序邀请码兑换走真实后端 `/api/waimao-mini/invite/redeem`，兑换后写入小程序专用 entitlement。
- 后端管理后台复用 Web 版 admin 登录，新增小程序专用的邀请码生成、列表、用户和统计接口，数据继续写入 `waimao_mini_*` 表。

### 🔧 Follow-up: 解锁页固定导航和渠道标签
...
- 课程详情字幕卡片的角色区分改回只使用左侧竖线颜色。
- 移除不同角色对整张字幕卡片的背景染色，避免遮挡当前播放/选中状态。
