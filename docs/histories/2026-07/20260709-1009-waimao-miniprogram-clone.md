...
### 🔧 Follow-up: 小程序主流程 UI 与功能提示重整

- 在不改变 EnglishPod 页面骨架和播放状态机的前提下，统一首页、课程详情、知识点、解锁、交流群和练习帮助的视觉层级与中文提示。
- 首页突出真实的“继续学习”位置，已解锁后即时隐藏会员入口；登录改为带明确上下文和行内错误的底部面板，不再自动弹出网页式练习说明。
- 课程详情将 Echo 对用户显示为“逐句跟读”，补齐模式、倍速、语言和播放状态反馈；角色只保留左侧色线，当前句使用独立选中态。
- 知识点对话改为优先读取服务端短字幕，逐条保留英文、中文和说话人对应关系，解决两种语言标点数量不同造成的错位。
- 解锁页按“会员权益 → 添加微信 → 输入邀请码”组织流程，兑换成功后展示精确到期日；邀请码错误改为输入框附近的可操作提示。
- 新增知识点字幕配对和解锁文案格式化测试；通过 TypeScript、全量测试和微信开发者工具主流程检查。
- 主页面全屏布局从 `page` 标签选择器迁移到 `.app-page` 根容器，并将新增的标签/属性后代选择器改为明确 class，消除组件化编译的 WXSS 选择器警告。

### 🔧 Follow-up: 登录与课程工具栏视觉收敛

- 登录底部面板改为固定圆形头像选择器、昵称字段和单一主操作，精简成“保存学习进度 / 继续解锁”的上下文文案；头像按钮增加严格宽高约束，避免微信原生按钮把图片拉成长方形。
- 课程顶部移除“影 / 句”方块字图标和说明小字，改为纯文字分段控件；首次提示收敛为“点按句子播放，长按单词查释义”。
- 课程底部讲解、重复、语言和知识点入口统一为 Lucide 线性图标，倍速直接展示数值，播放键保留为唯一填充主操作；图标在微信基础库 `3.7.12` 下完成渲染检查。
- 首页登录提示、课程分享卡和练习帮助分享文案统一使用“保存学习进度 / 逐句跟读”口径，不再向用户显示旧的 `Echo 模式` 名称。
- 默认分享图改为代码包内外贸品牌图标，后台播放封面改用外贸专用 `/static/waimao-mini/icon.png`；该资源已上传线上并校验与本地图标一致，不再读取 EnglishPod 通用图标地址。
