...
### 🔁 Follow-up: 重构为三 Tab 信息架构

用户进一步提出功能应采用多 Tab 方式。阶段 D 随后调整为三个原生一级入口：

- **课程**：保留继续学习、解锁提示、场景搜索和章节树。
- **复习**：生词与难句从“我的”下级入口提升为独立 Tab，访客也可使用本地资料。
- **我的**：集中学习记录、会员权益、练习方法与交流反馈；未登录时展示访客态，不自动跳页。

同时补充三组双态 PNG Tab 图标、访客登录回跳机制和 Tab 配置自动化测试。开发者工具已验证三 Tab 路由、选中态、复习空态和登录用户“我的”页面，问题面板为 0；线上学习记录接口未部署导致的 404 仍按计划留到部署后消除。

### 🔁 Follow-up: 修复学习记录并更新练习帮助

用户反馈最近学习记录未显示、练习方法内容过时，以及“练习方法 / 交流与反馈”入口图标不够清晰。本轮完成：

- 定位到线上学习记录接口仍为 404，而后端对应提交尚未部署；先备份生产数据库，再完成后端构建、重启与健康检查。开发者工具登录态复验后，“我的”页已恢复最近 4 周日历并显示 4 天活跃。
- 练习帮助页改为与课程播放器一致的“通听 → 精练 → 跟读”三阶段，补充卡住时的处理方式和有效练习标准；同步更新分享卡片文案与测试。
- 会员权益、练习方法、交流反馈入口改用语义明确的本地 SVG 线性图标，移除字符占位图标。
- 同步可靠性、前端约束、质量评分、执行计划和发布记录，消除“接口待部署”与旧 Shadow/Echo 方法说明。
