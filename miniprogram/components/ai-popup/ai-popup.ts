...
import { getAssistantDisplayState } from './render-state'
import { drainStreamingBuffer } from './stream-renderer'
...
Component({
  ...
  observers: {
    'show, text': function (show: boolean, text: string) {
  ...
  methods: {
    buildAssistantMessage(content: string, options?: {
      displayText?: string,
      isStreaming?: boolean,
      parsedNodes?: any[]
    }) {
    ...
    startStreamingRenderTimer(msgIndex: number, requestId: string) {
      const state = (this as any)
    ...
    enqueueStreamingText(textChunk: string, fullText: string, msgIndex: number, requestId: string) {
      if (!textChunk) return
    ...
    flushStreamingRender(forceAll: boolean) {
      const state = (this as any)
      ...
      const drained = drainStreamingBuffer({
    ...
    finalizeAssistantMessage(msgIndex: number, finalMessageContent: string) {
    ...
    onTouchStart(e: WechatMiniprogram.TouchEvent) {
      if (e.touches.length === 1) {
        (this as any).touchStartY = e.touches[0].clientY
        ;(this as any).lastTranslateY = 0
      }
    },
    ...
    onTouchMove(e: WechatMiniprogram.TouchEvent) {
      if (e.touches.length === 1) {
        ...
        const deltaY = clientY - (this as any).touchStartY
        ...
        if (deltaY > 0) {
          ...
          ;(this as any).lastTranslateY = deltaY
        }
      }
    },
    ...
    onTouchEnd() {
      ...
      const lastTranslateY = (this as any).lastTranslateY || 0
    ...
    animateHide() {
      ...
      animation.translateY('100%' as any).step()
      this.setData({
        animationData: animation.export()
      }, () => {
        setTimeout(() => {
    ...
    // 从缓存加载
    loadFromCache(text: string): any[] | null {
      try {
    ...
    // 保存到缓存
    saveToCache(text: string, messages: any[]) {
      try {
        const key = this.getCacheKey(text)
    ...
    // 检查并记录使用次数
    checkAndIncrUsage(callback?: () => void): boolean {
    ...
    // 播放激励视频广告
    playRewardVideoAd(callback?: () => void) {
    ...
    initConversation(text: string, context: string) {
      // 1. 先检查是否有缓存（缓存不计入次数）
    ...
    async requestAI(text: string, context?: string, isFollowUp?: boolean) {
      if (this.data.loading) return
      
      // 生成请求 ID，用于防止并发问题
      const requestId = Date.now().toString()
      // 保存当前请求的原始句子（用于缓存 key）
      const originalText = this.properties.text as string
      ...
      this.resetStreamingRenderState()
      ;(this as any).streamMessageIndex = msgIndex
      ;(this as any).streamRequestId = requestId
      ...
      try {
        ...
        // 类型定义可能未包含，使用 as any
        const requestUrl = `${API_BASE_URL}/api/explanations/stream`
        console.log('[Explain] request url', requestUrl)

        const requestTask = (wx as any).request({
          url: requestUrl,
          method: 'POST',
          data: requestBody,
          header: {
          ...
          success: (res: any) => {
            ...
            if (res.statusCode !== 200) {
              const errMsg = (res.data as any)?.error || `请求失败 (${res.statusCode})`
            ...
            const finalizedMessage =
              finalMessageContent && !finalMessageContent.includes('抱歉，')
        ...
        // 通过 onChunkReceived 逐块接收 SSE 数据
    ...
    extractTextFromResponseData(raw: any): { content: string, source: string } {
      if (raw == null) {
