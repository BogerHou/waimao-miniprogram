...
export type CompletionPosterStats = {
  totalCues: number
  practicedCount: number
}

export function buildCompletionStatsLabel(stats: CompletionPosterStats) {
  return stats.practicedCount > 0
    ? `共 ${stats.totalCues} 句 · 本次精练 ${stats.practicedCount} 句`
    : `共 ${stats.totalCues} 句`
}

export async function renderCourseCompletionPoster(
  scope: WechatMiniprogram.Page.Instance<any, any>,
  canvasId: string,
  courseTitle: string,
  stats: CompletionPosterStats,
) {
  ...
  ctx.setFontSize(25)
  drawShareWrappedText(ctx, courseTitle, WIDTH / 2, 262, WIDTH - 220, 36, 2)
