...
Page({
    ...
    async loadCourse(id) {
        ...
        try {
            ...
            const starredIds = new Set((0, practice_marks_1.getStarredCueIds)(this.readStarredCueMap(), detail.id));
            ...
            const shouldPreheatEchoAudio = modePresentation.showPracticeControls &&
        ...
        if (!this.audioContext) {
            ...
            audioContext.onPlay(() => {
                ...
                if (this.data.playMode === 'shadow' && this.data.isRepeating && this.activeSubtitle) {
                    ...
                    const stopWindow = playerCore.computeRepeatStopWindow({ start: subtitleStart, end: subtitleEnd }, this.data.playbackRate);
            ...
            audioContext.onError(err => {
                ...
                if (!this.usingFallbackAudio && this.data.playMode === 'echo' && this.activeSubtitle && this.data.course) {
                    ...
                    const segmentUrl = playerCore.buildEchoSegmentUrl(env_1.API_BASE_URL, this.data.course.id, this.activeSubtitle.id);
    ...
    restoreBackgroundAudioFromStorage() {
        ...
        const stageForShadow = playerCore.resolveStagePlan(this.data.stage, this.data.gapEnabled).channel === 'shadow'
            ? this.data.stage
            : 'listen';
        this.setData({
    ...
    applyPlayModeChange(mode, options = {}) {
    ...
    focusCueFromSource(cueId, autoplay) {
    ...
    ensureRecorderManager() {
        ...
        manager.onStop(res => {
            ...
            const forCurrentCue = Boolean(tempFilePath &&
                this.recordedCueId &&
    ...
    readStarredCueMap() {
        try {
            return (0, practice_marks_1.normalizeStarredCueMap)(wx.getStorageSync(practice_marks_1.STARRED_CUES_STORAGE_KEY));
        }
        catch (_error) {
            return {};
        }
    ...
    handleToggleStar(event) {
        ...
        const nextMap = (0, practice_marks_1.toggleStarredCue)(this.readStarredCueMap(), this.courseId, cueId);
        try {
    ...
    async openCompletionPanel() {
        ...
        try {
            ...
            nextScene = (0, next_scene_1.resolveNextScene)(response.data, this.courseId);
        }
        catch (error) {
    ...
    async generateCompletionShareImage() {
        ...
        try {
            const tempFilePath = await (0, course_completion_poster_1.renderCourseCompletionPoster)(this, COURSE_SHARE_CANVAS_ID, this.data.course.title, this.data.completionStats);
